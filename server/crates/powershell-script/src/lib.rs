...
pub use {builder::PsScriptBuilder, error::PsError, output::Output, psscript::PsScript};
...
/// ```
///
pub async fn run(script: &str) -> Result<Output> {
    PsScriptBuilder::default().build().run(script).await
}
